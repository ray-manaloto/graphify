    ...
    def _stub(paths, **kwargs):
        ...
        if be in fail_backends:
            return {"nodes": [], "edges": [], "hyperedges": [],
                    ...
                    "_execution_receipts": [{
                        "receipt_id": f"{be}-failed-before-response",
...
def test_unreceipted_primary_crash_blocks_fallback(monkeypatch, tmp_path, capsys):
    # A generic crash does not prove whether a provider response was produced.
    # The fallback must not risk re-spending after that evidence gap.
    calls = []
...
def test_receipted_pre_response_crash_allows_fallback(monkeypatch, tmp_path):
    calls = []
    error = RuntimeError("failed before response")
    error.graphify_attempt = {"receipt": {
        "receipt_id": "pre-response", "completion": "failed_before_response",
        "coverage": {"status": "complete", "reasons": []},
    }}
    stub = _recording_stub(calls, raise_backends={"claude": error})
    _arm(monkeypatch, tmp_path, stub, extra_argv=["--fallback-backend", "openai"])

    _run_ok()

    assert [call["backend"] for call in calls] == ["claude", "openai"]


def test_unreceipted_zero_output_blocks_fallback(monkeypatch, tmp_path, capsys):
    calls = []

    def _unreceipted(paths, **kwargs):
        calls.append({"backend": kwargs.get("backend")})
        return {"nodes": [], "edges": [], "hyperedges": [],
                "input_tokens": 0, "output_tokens": 0}

    _arm(monkeypatch, tmp_path, _unreceipted,
         extra_argv=["--fallback-backend", "openai"])

    with pytest.raises(SystemExit) as exc:
        mainmod.main()
    assert exc.value.code == 1
    assert [call["backend"] for call in calls] == ["claude"]
    assert "fallback blocked" in capsys.readouterr().err
...
def test_preserved_leaf_blocks_fallback_without_success_callback(
    monkeypatch, tmp_path, capsys
):
    calls = []

    def _preserved_leaf(paths, **kwargs):
        calls.append({"backend": kwargs.get("backend")})
        return {
            "nodes": [{
    ...
    _arm(
        monkeypatch, tmp_path, _preserved_leaf,
